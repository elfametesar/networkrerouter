...
fun CreateRuleDialog(
    ...
    val visibleApps = if (showSystemApps) apps else apps.filterNot { it.isSystemApp }
        ...
        Column(
            ...
            LazyColumn(modifier = Modifier.heightIn(max = 550.dp)) {
                ...
                item {
                    ...
                    Spacer(Modifier.height(6.dp))
                    InterfacePicker(interfaces, to, toMenuOpen, { toMenuOpen = it }) { to = it; toMenuOpen = false }
                }
                ...
                item {
                    Row(verticalAlignment = androidx.compose.ui.Alignment.CenterVertically) {
                        ...
                        Switch(checked = masquerade, onCheckedChange = { masquerade = it }, colors = SwitchDefaults.colors(checkedTrackColor = AccentSignal))
                    }
                }
                item { Spacer(Modifier.height(14.dp)) }
                item {
                    Row(verticalAlignment = androidx.compose.ui.Alignment.CenterVertically) {
                        Column(Modifier.weight(1f)) {
                            ...
                            Text(if (includeHotspot) "Hotspot traffic → tunnel" else "Only device local traffic → tunnel", style = AppType.dataSecondary)
                        }
                        Switch(checked = includeHotspot, onCheckedChange = { includeHotspot = it }, colors = SwitchDefaults.colors(checkedTrackColor = AccentSignal))
                    }
                }
                item { Spacer(Modifier.height(16.dp)) }
                item {
                    ...
                    Spacer(Modifier.height(6.dp))
                    Text("Exempt a proxy/VPN app to prevent deadlock when its own traffic routes into itself", style = AppType.dataSecondary)
                    Spacer(Modifier.height(8.dp))
                    OutlinedButton(onClick = { proxyMenuOpen = true }, modifier = Modifier.fillMaxWidth()) {
                        Text(selectedProxyApp?.label ?: "Select proxy app (optional)", style = AppType.dataPrimary)
                    }
                    if (proxyMenuOpen) {
                        AlertDialog(
                            ...
                            text = {
                                if (visibleApps.isEmpty()) Text("No apps found.", style = AppType.bodySecondary)
                                else LazyColumn(modifier = Modifier.fillMaxWidth().heightIn(max = 300.dp)) {
                                    items(visibleApps, key = { it.uid }) { app ->
                                        OutlinedButton(onClick = { selectedProxyApp = app; proxyMenuOpen = false }, modifier = Modifier.fillMaxWidth().padding(vertical = 3.dp), shape = RoundedCornerShape(10.dp)) {
                                    ...
                                    item {
                                        OutlinedButton(onClick = { selectedProxyApp = null; proxyMenuOpen = false }, modifier = Modifier.fillMaxWidth().padding(vertical = 8.dp), shape = RoundedCornerShape(10.dp)) {
                                            Text("None (no exemption)", style = AppType.dataPrimary)
                                        }
                                    }
                                }
                            },
                            confirmButton = { TextButton(onClick = { proxyMenuOpen = false }) { Text("Close") } }
                        )
                    }
                }
                item { Spacer(Modifier.height(14.dp)) }
                item {
                    Row(verticalAlignment = androidx.compose.ui.Alignment.CenterVertically) {
                        Column(Modifier.weight(1f)) {
                            Text("Show system apps", style = AppType.body)
                            Text(if (showSystemApps) "System apps are included" else "Only user apps are shown", style = AppType.dataSecondary)
                        }
                        Switch(checked = showSystemApps, onCheckedChange = { showSystemApps = it }, colors = SwitchDefaults.colors(checkedTrackColor = AccentSignal))
                    }
                }
                ...
                item {
                    ...
                    Spacer(Modifier.height(6.dp))
                    if (visibleApps.isEmpty()) Text("No apps found", style = AppType.bodySecondary, modifier = Modifier.padding(vertical = 10.dp))
                    else LazyColumn(modifier = Modifier.heightIn(max = 200.dp)) {
                        items(visibleApps, key = { it.uid }) { app ->
                            AppExcludeRow(app, app.uid in excluded) { checked ->
            ...
            Row(horizontalArrangement = Arrangement.End, modifier = Modifier.fillMaxWidth()) {
                ...
                OutlinedButton(enabled = from.isNotEmpty() && to.isNotEmpty() && from != to, onClick = {
                    onConfirm(from, to, excluded, masquerade, selectedProxyApp?.packageName, if (includeHotspot) SourceInterfaceType.LOCAL_AND_HOTSPOT else SourceInterfaceType.LOCAL_ONLY)
                }) { Text("Create route") }
            }
        }
    }
...
private fun InterfacePicker(
    interfaces: List<NetInterface>, selected: String, expanded: Boolean,
    onExpandedChange: (Boolean) -> Unit, onSelect: (String) -> Unit
) {
    ...
    if (expanded) {
        AlertDialog(
            ...
            text = {
                if (interfaces.isEmpty()) Text("No network interfaces found.", style = AppType.bodySecondary)
                else LazyColumn(modifier = Modifier.fillMaxWidth().heightIn(max = 420.dp)) {
                    items(interfaces, key = { it.name }) { iface ->
                        OutlinedButton(onClick = { onSelect(iface.name) }, modifier = Modifier.fillMaxWidth().padding(vertical = 3.dp), shape = RoundedCornerShape(10.dp)) {
                            Column(modifier = Modifier.fillMaxWidth()) {
                                ...
                                }.ifEmpty { "DOWN / no IPv4" }, style = AppType.dataSecondary)
                            }
                        }
                    }
                }
            },
            confirmButton = { TextButton(onClick = { onExpandedChange(false) }) { Text("Close") } }
        )
    }
}
...
private fun AppExcludeRow(app: AppInfo, checked: Boolean, onCheckedChange: (Boolean) -> Unit) {
    Row(verticalAlignment = androidx.compose.ui.Alignment.CenterVertically, modifier = Modifier.fillMaxWidth().padding(vertical = 2.dp)) {
        Checkbox(checked = checked, onCheckedChange = onCheckedChange, colors = CheckboxDefaults.colors(checkedColor = AccentSignal))
        Column {
