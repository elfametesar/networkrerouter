        ...
        true -> Scaffold(
            ...
            LazyColumn(
                ...
                items(state.interfaces, key = { it.name }) { iface ->
                    InterfaceCard(
                        iface = iface,
                        onToggle = { up -> viewModel.toggleInterfaceState(iface.name, up) },
                ...
                if (state.interfaces.isEmpty()) {
                    item { EmptyRow("No interfaces found") }
                }
                ...
                items(state.rules, key = { it.id }) { rule ->
                    ...
                    RuleCard(
                        ...
                        onToggleEnabled = { enabled -> viewModel.toggleRuleEnabled(rule.id, enabled) },
                        onRemove = { viewModel.removeRule(rule.id) }
                    )
                }
                if (state.rules.isEmpty()) {
                    item { EmptyRow("No routes yet — traffic follows the system default") }
                }

                item { Spacer(Modifier.height(16.dp)) }
                item {
                    ResetAllRow(onClick = { showResetConfirm = true })
                }
            }
        }
    }
...
private fun SectionHeader(title: String) {
    Text(title.uppercase(), style = AppType.sectionLabel.copy(color = TextSecondary))
}

@Composable
private fun EmptyRow(text: String) {
    Box(modifier = Modifier.fillMaxWidth().padding(vertical = 12.dp)) {
        Text(text, style = AppType.dataSecondary)
    }
}
...
private fun ResetAllRow(onClick: () -> Unit) {
    Row(
        ...
        androidx.compose.material3.TextButton(onClick = onClick) {
            Icon(Icons.Outlined.RestartAlt, contentDescription = null, tint = AccentDanger)
...
private fun NoRootScreen() {
    Box(modifier = Modifier.fillMaxSize().padding(32.dp), contentAlignment = Alignment.Center) {
        Column(horizontalAlignment = Alignment.CenterHorizontally) {
            Text("Root required", style = AppType.displayTitle, fontWeight = FontWeight.SemiBold)
            Spacer(Modifier.height(8.dp))
            Text(
                "This app manages interfaces and routing tables directly, which needs root access. " +
                    "Grant root in your root manager (Magisk, KernelSU) and reopen the app.",
