...
class RoutingEngine {
    ...
    private suspend fun findFreePriority(min: Int, max: Int, reserved: Set<Int> = emptySet()): Int? {
        val used = ipRuleLines().mapNotNull { it.substringBefore(":").trim().toIntOrNull() }.toSet() + reserved
        return (max downTo min).firstOrNull { it > 0 && it !in used }
    }

    /** First Android rule that can consume marked local application traffic. */
    ...
    /**
     * Reserve consecutive free priorities immediately before Android's first
     * fwmark/iif-lo policy rule. This is discovered from live kernel state;
     * no device-specific priority is hardcoded.
     */
    private suspend fun findLocalEgressPriorities(count: Int): List<Int>? {
        if (count <= 0) return emptyList()
        val ceiling = findLocalPolicyCeiling() ?: return null
        val used = ipRuleLines().mapNotNull { it.substringBefore(":").trim().toIntOrNull() }.toSet()
        val result = mutableListOf<Int>()
        var cursor = ceiling - 1
        while (cursor > 0 && result.size < count) {
            if (cursor !in used) result.add(cursor)
            cursor--
        }
    ...
    private suspend fun applyLocalEgressRule(rule: RouteRule, table: Int, realRoutingTable: String?): List<String>? {
        val priorityCount = rule.excludedUids.size + 1
        val priorities = findLocalEgressPriorities(priorityCount) ?: return null
        val commands = mutableListOf<String>()

        // Remove stale copies of this route first. This also cleans rules made
        // by older releases that used a later fixed priority.
        findIifLoTableRules(table).forEach { old ->
            commands.add("ip rule del priority $old 2>/dev/null || true")
        }

        rule.excludedUids.forEachIndexed { index, uid ->
        ...
        val tunnelPriority = priorities.last()
    ...
    suspend fun applyRule(rule: RouteRule, realRoutingTable: String? = null): RuleApplyResult {
        ...
        if (hotspot != null) {
            ...
            val localCommands = applyLocalEgressRule(rule, table, realRoutingTable)
                ?: return RuleApplyResult(false, stderr = "Could not allocate free priorities before Android's first fwmark/iif-lo rule")
